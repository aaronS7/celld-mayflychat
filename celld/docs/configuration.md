...
| `TYPESAFE_API_KEY` | Unset; nonblank TypeSafe credential | Needed only for plaintext with Jev enabled. Leading/trailing whitespace is trimmed. Missing credentials or provider failures refuse posts with HTTP 503 `moderation_unavailable`; they do not disable screening. Never commit a real key. |
| `TYPESAFE_MODEL` | `jev-latest`; a model identifier supported by TypeSafe | Passed to the provider. An empty string also selects `jev-latest`. A moving alias may change detection behavior; use a supported fixed model identifier when reproducibility matters. Invalid/unavailable models fail closed when the provider refuses them. |
...
The actual TypeSafe endpoint, 70% threshold, and ten-second provider timeout are
fixed in the application; there are no environment overrides for them.

## Encryption and moderation policy

| Encryption | Jev | New messages |
| --- | --- | --- |
| `0` | `0` | Plaintext, without provider calls |
| `0` | `1` | Plaintext, screened before append |
| `1` | Any | Client-encrypted; no provider calls |
...
or remove old messages. Previously stored plaintext and provider copies do not
...
Screening adds an external dependency, latency, and API usage. It classifies
...
| `ENCRYPTION_ENABLED`, `JEV_ENABLED`, `TYPESAFE_MODEL` | A supplied shell variable overrides the previously saved selection, which overrides `wrangler.jsonc`. Omission keeps the previous selection; use explicit `0` to disable a flag. A model reset can use `TYPESAFE_MODEL=jev-latest`. |
| `TYPESAFE_API_KEY` | For plaintext screening, a nonempty shell value takes precedence over the `TYPESAFE_API_KEY=...` line in ignored `typesafe.celld.env`. The file supports optional enclosing quotes, not shell execution. Missing, blank, or multiline resolved key values stop deployment; provider validity is checked only when screening a post. The key is loaded again for every screened deployment; it is not saved in `fleet.json`. |

The helper only forwards the three policy/model variables and credential above.
...
test deployments; targeting a screened server can consume real API calls. The
