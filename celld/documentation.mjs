...
## Keys and envelopes`);
  ...
  docs["docs/browser.md"] += "\n## File downloads and video\n\nChat images and common file links have Download / open controls. Direct MP4, WebM and Ogg video links offer Load video, native play/pause, seeking, volume and Fullscreen. Media loads only after a click; videos do not autoplay. External hosts may open a viewer instead of a download, in which case use the browser's Save command. Mayfly does not proxy external files or send them a chat bearer. Chat has no upload storage. Wiki editors can attach files up to 5 MiB, with authenticated Download controls preserving filenames, inline image previews and on-demand video playback. Other files, including HTML and SVG, are download-only. See [wiki attachments](wiki.md#browser-workflow).\n";
