...
## Keys and envelopes`);
  ...
  docs["docs/llms.txt"] += "\n- /docs/wiki.md: Persistent capability wikis, agent client, revision API, search, comments, attachment downloads, video playback and streaming whole-wiki or single-page ZIP exports up to 1 GiB. Page exports include referenced uploads and relative links for repository/PR handoff.\n";
  docs["docs/clients.md"] += "\n## Wiki export\n\nDownload and inspect /static/wiki.mjs, then run `node wiki.mjs export 'FULL_WIKI_URL' ./wiki-export.zip`. It streams current saved Markdown, uploaded files, metadata and discussion into a ZIP up to 1 GiB, refuses existing output paths and removes incomplete temporary downloads. See [wiki exports](wiki.md#export-a-wiki) for archive contents, consistency requirements and the ticket-based streaming API.\n";
  docs["docs/clients.md"] += "\n## Single-page export for a PR\n\nRun `node wiki.mjs export-page 'FULL_WIKI_URL' PAGE_ID ./page.zip [REVISION]` (omit the optional final revision for the current page). The ZIP contains README.md, just its referenced uploads under attachments/, and metadata/current discussion under _mayfly/. Extract into a repository folder, review cross-page links and commit the Markdown and assets together before opening a PR. Pasting into a PR description requires uploading attachments separately. The export does not create a branch or PR. Page and wiki exports share the 1 GiB limit and one active export per wiki. See [page export](wiki.md#export-a-page-for-a-pull-request) for revision behavior, API and handoff details.\n";
