...
export async function nativeDocumentation(upstream) {
  const docs = { ...upstream };
  for (const name of ["hosting", "operations", "configuration", "tagging"]) docs[`docs/${name}.md`] = await readFile(new URL(`./docs/${name}.md`, import.meta.url), "utf8");
...
the channel's encryption mode, effective moderation/tagging policy, and postingAllowed.
