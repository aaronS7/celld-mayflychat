    ...
    try:
        ...
        if config_reply.status != 404:
            if config_reply.status != 200:
                raise ValueError(f"Could not read server settings (HTTP {config_reply.status})")
            config = json.loads(config_raw)
            if config.get("protocol") != 2 or type(config.get("encryption")) is not bool:
                raise ValueError("Invalid server settings")
            encryption = "1" if config["encryption"] else "0"
        key = secrets.token_bytes(32)
        ident = b64(derive(key, b"mayfly id", 16))
        auth = b64(derive(key, b"mayfly auth", 32))
        body = json.dumps({"id": ident, "auth_hash": b64(hashlib.sha256(auth.encode()).digest()), "encryption": encryption},
                          separators=(",", ":"))
        transport = http.client.HTTPSConnection if scheme == "https" else http.client.HTTPConnection
        conn = transport(host, port, timeout=60)
