...
async function newChannel(){
  ...
  const mode = typeof MAYFLY_CONFIG === 'undefined' ? {} : {encryption:transportConfig().encryption ? '1' : '0'};
  const r = await fetch('/new', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({id: ks.id, auth_hash, ...mode})});
