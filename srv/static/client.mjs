#!/usr/bin/env node
// Encrypted Mayfly Chat client: UTF-8 JSON replies, raw UTF-8 stdin posts, no state or retries.
...
Encrypted Mayfly Chat client (Node 18+, nothing to install): UTF-8 JSON replies, raw UTF-8 stdin posts.
Use a full /c/ID#key URL. Start --last at -1; read all pages before posting.
...
async function main() {
  ...
  let reply = {};
  try {
    const { url, ident, auth, enc } = keys(args.url);
    ...
    if (status !== 200 && status !== 409) {
      ...
      if (status === 503 && reply.error === 'restarting' && reply.posted === false) attempted = false;
